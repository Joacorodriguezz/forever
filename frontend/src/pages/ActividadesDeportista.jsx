...
function ActividadesDeportista() {
  ...
  useEffect(() => {
    ...
    if (usuarioStr) {
      try {
        ...
        setDeportistaId(usuario?.deportista?.id ?? null);
      } catch (e) {
    ...
    const fetchAll = async () => {
      ...
      try {
        const [resActs, resInsc] = await Promise.all([
          ...
          axios.get(`${BACKURL}/api/actividadDeportista/deportista/${deportistaId}`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleInscribirse = async (actividadId) => {
    try {
      const res = await axios.post(
        `${BACKURL}/api/actividadDeportista`,
        { actividadId, deportistaId },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      const nuevaInscripcion = res?.data?.actividadDeportista ?? res?.data;
  ...
  const handleBaja = async (actividadId) => {
    ...
    try {
      await axios.delete(`${BACKURL}/api/actividadDeportista/${insc.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
export default ActividadesDeportista;
